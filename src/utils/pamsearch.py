...
def adjust_algorithm(alg, best_param, idx, timepam, blocksize):
    detector = None
    if alg == 'sdo':
        detector = pysdo.SDO(k=best_param['k'], x=best_param['x'], qv=best_param['qv'],
                             random_state=idx, chunksize=blocksize+1, return_scores=True)
    elif alg == 'sdostream':
        detector = outlier.SDOstream(
            k=best_param['k'], T=timepam, x=best_param['x'], qv=best_param['qv'], seed=idx)
    elif alg == 'swknn':
        detector = outlier.SWKNN(k=best_param['k'], window=timepam)
    elif alg == 'swlof':
        detector = outlier.SWLOF(k=best_param['k'], window=timepam)
    elif alg == 'rrct':
        detector = outlier.SWRRCT(
            n_estimators=best_param['n_estimators'], window=timepam, seed=idx, n_jobs=1)
    elif alg == 'xstream':
        detector = xStream(k=best_param['k'], c=best_param['c'],
                           d=best_param['d'], init_sample=timepam, seed=idx)
    elif alg == 'rshash':
        detector = outlier.RSHash(
            n_estimators=best_param['n_estimators'], cms_w=best_param['cms_w'], cms_d=best_param['cms_d'], window=timepam, seed=idx, n_jobs=1)
    elif alg == 'loda':
        detector = outlier.LODA(
            n_projections=best_param['n_projections'], n_bins=best_param['n_bins'], window=timepam)
    return detector
...
def parameter_selection(data, labels, timestamps, training_len, blocksize, alg, timepam):
    if alg == 'sdo':
        ...
        param = dict(k=pk, x=px, qv=pqv, chunksize=[
                     blocksize+1], return_scores=[True], random_state=[0])
        wrapper = wSDO()
        best_param = parameter_tuning(param, np.hstack(
            (data[0:training_len, :], timestamps[0:training_len])), labels[0:training_len], wrapper)
    elif alg == 'sdostream':
        ...
        wrapper = wSDOstream()
        best_param = parameter_tuning(param, np.hstack(
            (data[:training_len, :], timestamps[:training_len])), labels[:training_len], wrapper)
    elif alg == 'swknn':
        print("Algorithm: SWKNN")
        best_param = sw_gridsearch(outlier.SWKNN, timepam, 50, np.hstack(
            (data[:training_len, :], timestamps[:training_len])), labels[:training_len])
    elif alg == 'swlof':
        print("Algorithm: SWLOF")
        best_param = sw_gridsearch(outlier.SWLOF, timepam, 35, np.hstack(
            (data[:training_len, :], timestamps[:training_len])), labels[:training_len])
    elif alg == 'rrct':
...
def parameter_selection_2D(data, labels, timestamps, training_len, blocksize, alg, timepam):
    if alg == 'sdo':
        ...
        print("Algorithm: SWKNN")
        best_param = sw_gridsearch(outlier.SWKNN, timepam, 50, np.hstack(
            (data[0:training_len, :], timestamps[0:training_len])), labels[0:training_len])
    elif alg == 'swlof':
        print("Algorithm: SWLOF")
        best_param = sw_gridsearch(outlier.SWLOF, timepam, 35, np.hstack(
            (data[0:training_len, :], timestamps[0:training_len])), labels[0:training_len])
...
def compute_score(y_true, scores):
    ...
    ap = np.sum(y_true_cs[:num_outliers] /
                np.arange(1, num_outliers + 1)) / num_outliers
...
def sw_gridsearch(detector, window, k_max, X, y):
    ...
    for _, test_indices in cv.split(X, y):
        ...
        results += [compute_training_score(y[test_indices],
                                           transform_scores(scores[:, i])) for i in range(k_max)]
