...
class SDOs(Model):
    ...
    def train(self, X_train, **kwargs):
        ...
        best_param = pams.parameter_selection(
            X_train, labels, timestamps, self.training_len, blocksize, 'sdostream', self.observation_period)
        print('Best parameter:', best_param)
        self.detector = pams.adjust_algorithm(
            'sdostream', best_param,  self.idx, self.observation_period, blocksize)
