...
class RSHASH(Model):
    ...
    def train(self, X_train, **kwargs):
        ...
        best_param = pams.parameter_selection_2D(
            X_train, labels, timestamps, self.training_len, blocksize, 'rshash', self.observation_period)
        print('Best parameter:', best_param)
        self.detector = pams.adjust_algorithm(
            'rshash', best_param,  self.idx, self.observation_period, blocksize)
