...
class LODASALMON(Model):
    """LODA implementation using dSalmon library. Implemented for validation purposes.""" 
    ...
    def train(self, X_train, **kwargs):
        ...
        best_param = pams.parameter_selection(
            X_train, labels, timestamps, self.training_len, blocksize, 'loda', self.observation_period)
        print('Best parameter:', best_param)
        self.detector = pams.adjust_algorithm(
            'loda', best_param,  self.idx, self.observation_period, blocksize)
    ...
    def predict(self, X):
        ...
        self.scores = pams.transform_scores(self.scores)
        return self.scores
